        ...
        <div className=' rounded-2xl w-[95%] p-4 text-white py-0 mb-10'>
          <div className='flex flex-wrap justify-center gap-8'>
           {isLoding ? (games.map((game,index) => (
              <div key={index} className='slides invisible translate-y-5'><SlideGames id={game.id} url={game.background_image} title={game.name} platform={game.parent_platforms} rating={game.rating} /></div>
           )) ) : (games.map((game,index) => (
              <div key={index} className='slides invisible translate-y-5'><div className="h-80 w-80 rounded-2xl bg-white/5 animate-pulse" /></div>
           )) )}
          </div>
