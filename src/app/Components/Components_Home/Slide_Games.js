    ...
    <div className='flex flex-col gap-2 my-3'>
      ...
      */}
      <div className='group relative cursor-pointer transition-transform duration-300 ease-out hover:scale-102 active:scale-95'>
        ...
        <div class="absolute inset-0 rounded-3xl bg-gradient-to-t from-black/90 via-black/30 to-transparent opacity-70 transition-opacity duration-500 group-hover:opacity-90"></div>

        <div class="absolute bottom-4 z-40 left-1 right-0 p-5 transform transition-transform duration-500 ease-out translate-y-2 group-hover:translate-y-0">
          <h3 class="text-white text-xl font-bold mb-1 drop-shadow-lg">{game.name}</h3>
          <div class="flex items-center gap-2 opacity-100 group-hover:opacity-100 transition-opacity duration-500 delay-100">
            <span class="text-red-400 text-sm font-semibold">RPG</span>
            <span class="text-gray-400 text-xs">•</span>
            <span class="text-yellow-400 text-sm font-bold">{`⭐ ${game.rating}`}</span>
          </div>
        </div>
